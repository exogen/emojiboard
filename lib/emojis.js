 ...
 */
async function getEmojis({ userId, emoji: emojiName, count = 10 } = {}) {
  const now = Date.now();
  ...
  if (lastResponse && now - lastResponse.timestamp < ttl) {
    ...
    console.log("Using cached response! Age:", now - lastResponse.timestamp);
  } else {
    console.log("Requesting fresh response...");
    emojis = new Map();
    try {
      let done = false;
      let page = 1;
      do {
        const response = await api("/emoji.adminList", {
          searchParams: { page, count: 1000 }
        });
